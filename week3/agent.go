...
func operationActive(operation *Operation) bool {
	return operation != nil && (operation.State == "running" || operation.State == "compressing" || operation.State == "awaiting_tool_approval")
}
...
func NewAgent(cfg Config, provider any, logger *DebugLogger) (*Agent, error) {
	...
	if native, ok := provider.(NativeToolProvider); ok {
		a.native = native
	}
...
func (a *Agent) run(ctx context.Context, id, opID string) {
	...
	var err error
	nativeUsed := a.native != nil && (len(a.MCP.List()) > 0 || len(op.ToolMessages) > 0)
	if nativeUsed {
		completion, err = a.completeWithMCP(ctx, id, opID, messages, phaseSettings)
	} else {
	...
	r.cancel = nil
	if r.session.Operation.State == "completed" || r.session.Operation.State == "truncated" {
		r.session.Operation.ToolMessages = nil
		r.session.Operation.PendingTool = nil
		r.session.Operation.PendingCalls = nil
		r.session.Operation.ToolApproved = nil
		r.session.Operation.InFlightTool = ""
	}
	now := time.Now().UTC()
